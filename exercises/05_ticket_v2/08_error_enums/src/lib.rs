...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(TicketNewError::DescriptionError(_)) => {
            let new_description = String::from("Description not provided");
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError("Title cannot be empty".parse().unwrap()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError("Title cannot be longer than 50 bytes".parse().unwrap()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError("Description cannot be empty".parse().unwrap()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError("Description cannot be longer than 500 bytes".parse().unwrap()));
        }

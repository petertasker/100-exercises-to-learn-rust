...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(NewDescriptionError::DescriptionEmptyError("The description cannot be empty".to_string()));
        }
        if value.len() > 500 {
            return Err(NewDescriptionError::DescriptionTooLongError("The description cannot be longer than 500 bytes".to_string()));
        }
        Ok(TicketDescription(value.parse().unwrap()))
    }
}
...
#[derive(Debug)]
pub enum NewDescriptionError {
    DescriptionEmptyError(String),
...
impl Display for NewDescriptionError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            NewDescriptionError::DescriptionEmptyError(msg) => write!(f, "{}", msg),
            NewDescriptionError::DescriptionTooLongError(msg) => write!(f, "{}", msg),
        }
    }
}


impl Error for NewDescriptionError {
    fn description(&self) -> &str {
        match self {
            NewDescriptionError::DescriptionEmptyError(_) => "The description cannot be empty",
            NewDescriptionError::DescriptionTooLongError(_) => "The description cannot be longer than 500 bytes",
        }
    }
}

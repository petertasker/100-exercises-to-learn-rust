...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            return Err("Title cannot be empty".parse().unwrap())
        }
        if title.len() > 50 {
            return Err("Title cannot be longer than 50 bytes".parse().unwrap());
        }
        if description.is_empty() {
            return Err("Description cannot be empty".parse().unwrap());
        }
        if description.len() > 500 {
            return Err("Description cannot be longer than 500 bytes".parse().unwrap());
        }

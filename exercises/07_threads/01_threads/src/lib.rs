...
pub fn sum(v: Vec<i32>) -> i32 {
    let mid_idx = v.len() / 2;
    let left_vec = v[0..mid_idx].to_vec();
    let right_vec = v[mid_idx..].to_vec();

    let left_summer = thread::spawn(move || left_vec.iter().sum::<i32>());
    let right_summer = thread::spawn(move || right_vec.iter().sum::<i32>());
